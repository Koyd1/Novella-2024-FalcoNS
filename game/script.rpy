...
label par_house_td: # Расскажите о дочери
    ...
    show mrs_lawrence at halfed_right, darken
    mc_thoughts "Ее дочери нет в живых, а она вспоминает о конкурсах"
    $unlock_clue("parents_house_mc5")
...
label par_house_wuc: # Вы были близки с ней
    ...
    show mr_lawrence at halfed_left, lighten
    $unlock_clue("kid_album")
    mr_lawrence @ sad "Вот, посмотрите. Она всегда была красивой девочкой."
    $unlock_clue("adult_album")
    mr_lawrence @ sad "А здесь она уже роскошная девушка (показывает выпускной альбом). Не представляю, кто мог так поступить с ней... Только чудовище. Она сама никогда бы не сделала это..."
...
label par_house_whss: # С кем делилась переживаниями
    ...
    show mr_lawrence at halfed_left, lighten
    $unlock_clue("kid_album")
    mr_lawrence "Вот, посмотрите. Она всегда была красивой девочкой."
    $unlock_clue("adult_album")
    mr_lawrence"А здесь она уже роскошная девушка . Не представляю, кто мог так поступить с ней... Только чудовище. Она сама никогда бы не сделала это..."
...
label hosp_lms: # Давайте посмотрю
    ...
    with dissolve
    dr_andrews "Смерть наступила около полуночи вследствие убийства. Следов сексуального насилия не обнаружено."
    $unlock_clue("hospital_mc4")
...
label hosp_oc: # Только заключение
    dr_andrews "Смерть наступила около полуночи вследствие убийства. Следов сексуального насилия не обнаружено. Отпечатков также не обнаружено из-за нахождения тела в воде. Причиной смерти является удушье, но так как в легких вода не была обнаружена, она не дышала еще до того, как оказалась в реке. Жертва была задушена собственным шарфом."
    $unlock_clue("hospital_mc1")
    $unlock_clue("hospital_mc4")
    scene corpse_clothes
    with dissolve
    dr_andrews "Ее одежда здесь, можешь забрать орудие убийства. Родители не захотели забирать ее вещи."
    mc "Интересная семья..."
    scene hospital
    with dissolve
    show dr_andrews at halfed
    ...
    menu:
        ...
        "Вау, мы сузили круг до всех мужчин от 16 до 70":
            ...
            mc "Пожалуй взгляну."
            $ unlock_achievement( "objects", "2.1", "Вы получили достижение!")
            jump hosp_lc
...
label hosp_lm: # Узнать больше
    scene corpse_clothes
    with dissolve    
    dr_andrews "Смерть наступила вследствие удушья. Жертва была задушена собственным шарфом. Ее одежда здесь, можешь забрать орудие убийства. Родители не захотели забирать ее вещи."
    $unlock_clue("hospital_mc2")
...
label restaurant:
    ...
    menu:
        ...
        "Это важно для расследования и займет всего пять минут вашего времени.":
            ...
            jump rest_cwl2
        "Я имею все полномочия задержать вас за сопротивление следствию и вместо пяти минут, допрос будет длится несколько часов.":
            $ unlock_achievement( "leadership", "5.4", "Вы получили достижение!")
            jump rest_angry

label rest_angry: # я имею все полномочия задержать вас за сопротивление следствию и вместо пяти минут, допрос будет длиться несколько часов
    ...
    mrs_velaskez @ irritated "Воровство уже давно не подозрительно в этом коллективе посудомоек."
    menu:
        " "
        "* Расспросить далее *":
            jump rest_am
...
label rest_whn: # # Что произошло после
    lisa "Ее спутник был не в восторге от произошедшего. Сначала ушел скандалист, бросив в сторону девушки кольцо, которое видимо предназначалось ей, а потом и ее парень встал, расплатился и ушел, оставив ее одну."
    $unlock_clue("restaurant_mc1")
    $unlock_clue("restaurant_mc2")
...
label james_house:
    ...
    menu:
        ...
        "Вы были с ней в ресторане вечером 17-ого числа?":
            ...
            james @ smile "Мы были вместе почти 3 месяца, и это были самые прекрасные дни в моей жизни."
            $unlock_clue("james_house_mc6")
            mc "Расскажите, что произошло в тот вечер."
            jump jhouse_twhte
...
label evidence_exchange:
    ...
    if main_location !="Больница":
        ...
        $unlock_clue("hospital1")
        $unlock_clue("hospital2")
    ...
    if main_location !="Дом родителей":
        ...
        if assigned_person == "govard":
            ...
            $unlock_clue("parents_house_black")
        ...
        $unlock_clue("parents_house1_1")
        $unlock_clue("parents_house1_2")
        $unlock_clue("parents_house2_1")
        $unlock_clue("parents_house2_2")
    ...
    if main_location !="Университет":
        ...
        $unlock_clue("university1")
        $unlock_clue("university2_1")
        $unlock_clue("university2_2")
        $unlock_clue("university3")
        $unlock_clue("university4")
...
label evidence_second_exchange:
    ...
    if main_location !="Дом на\nАрмори-стрит 19":
        ...
        if assigned_person == "govard":
            ...
            $unlock_clue("james_house_govard")
            show govard at sprite_centered, darken
        if assigned_person == "phil":
            show phil at sprite_right, lighten
            ...
            $unlock_clue("james_house_phil")
            show phil at sprite_right, darken
        $unlock_clue("james_house1")
        $unlock_clue("james_house2_1")
        $unlock_clue("james_house2_2")
    ...
    if main_location !="Дом на\nМарч-драйв 77":
        ...
        if assigned_person == "phil":
            ...
            $unlock_clue("kyle_house_phil")
            show phil at sprite_right, darken
        $unlock_clue("kyle_house1_1")
        $unlock_clue("kyle_house1_2")
        $unlock_clue("kyle_house2_1")
        $unlock_clue("kyle_house2_2")
    ...
    if main_location !="Ресторан на\nБлинк-роуд":
        ...
        if assigned_person == "phil":
            ...
            $unlock_clue("restaurant_phil")
            show phil at sprite_right, darken
            $unlock_person("lisa", "phil")#!!!
        $unlock_clue("restaurant1")
        $unlock_clue("restaurant2_1")
        $unlock_clue("restaurant2_2")
