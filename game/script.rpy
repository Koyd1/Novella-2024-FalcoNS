...
label hosp_oc: # Только заключение
    dr_andrews "Смерть наступила около полуночи вследствие убийства. Следов сескуального насилия не обнаружено. Отпечатков также не обнаружено из-за нахождения тела в воде. Причиной смерти является удушье, но так как в легких вода не была обнаружена, она не дышала еще до того, как оказалась в реке. Жертва была задушена собственным шарфом."
    $unlock_clue("hospital_mc1")
    $unlock_clue("hospital_mc2")
    $unlock_clue("hospital_mc4")
    scene corpse_clothes
    with dissolve
    dr_andrews "Ее одежда здесь, можешь забрать орудие убийста. Родители не захотели забирать ее вещи."
    mc "Интересная семья..."
    scene hospital
    with dissolve
    show dr_andrews at halfed
...
label evidence_exchange:
    ...
    if main_location !="Дом родителей":
        ...
        $unlock_clue("parents_house1")
        $unlock_clue("parents_house2")
...
label evidence_second_exchange:
    ...
    if main_location !="Дом на\nМарч-драйв 77":
        ...
        if assigned_person == "phil":
            ...
            show phil at sprite_right, darken
        $unlock_clue("kyle_house1")
        $unlock_clue("kyle_house2")
